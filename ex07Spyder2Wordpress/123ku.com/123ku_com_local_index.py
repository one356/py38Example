...
def spyder_magnet(page_url):
    ...
    for i in range(0, len(div)):
        url_a = 'http://www.123ku.com' + div[i].find('a').attrs['href']
        print(page_url)
        print(i)
        # 防止被封，设置不定时间隔
        ...
        title_replace = title.replace('123酷资源站', 'magnetkey.xyz')
        # print(title)
        # 获取图片src
        ...
        # 获取链接地址
        div_magnet = soup.find_all(name='div', attrs={'class': 'ibox playBox'})[2]
        # 上传信息到wordpress
...
def multi_thread():
    ...
    for thread in threads_list:
        # 设置上传速度，否则会无法上传
        time.sleep(random.randint(90, 140))
        thread.start()
    for thread in threads_list:
        thread.join()
